...
import GhaziabadLocalityClientPage from "./page.client";
...
export async function generateMetadata({ params }: { params: { locality: string } }): Promise<Metadata> {
  ...
  return {
    title: `Bike Mechanic Near Me in ${data.name}, Ghaziabad – Doorstep Bike Repair | FixWheel`,
    description: `Looking for a bike mechanic near me in ${data.name}, Ghaziabad? FixWheel dispatches certified mechanics to your doorstep in ${data.eta} for oil change, brake repair, battery replacement & full service. Call +91-87459-45682. Starting ₹399.`,
    keywords: [
      `bike mechanic near me in ${data.name}`,
      `bike mechanic near me in ${data.name} Ghaziabad`,
      ...
      `two wheeler mechanic near me in ${data.name} Ghaziabad`,
      `scooty repair near me ${data.name}`,
      `doorstep bike repair near me ${data.name}`,
      `doorstep bike service near me ${data.name}`,
      `bike service at home in ${data.name} Ghaziabad`,
      `bike mechanic in ${data.name}`,
      `two wheeler mechanic in ${data.name} Ghaziabad`,
      ...
      `scooter mechanic ${data.name} Ghaziabad`,
      `scooty mechanic at home ${data.name}`,
      `scooter repair at home ${data.name} Ghaziabad`,
      `bike service at home in ${data.name}`,
      `engine oil change ${data.name}`,
      `Activa 6G service ${data.name} Ghaziabad`,
      `Activa 125 repair near me ${data.name}`,
      `Splendor Plus service ${data.name} Ghaziabad`,
      ...
      `Yamaha R15 V4 service ${data.name} Ghaziabad`,
      ...
      `two wheeler mechanic in ${data.name} Ghaziabad with phone number`,
      "doorstep two wheeler service Ghaziabad",
      "mobile bike mechanic Ghaziabad",
      "two wheeler repair at home Ghaziabad",
      `home bike service ${data.name}`,
      "bike mechanic near me Ghaziabad"
    ],
    alternates: {
      canonical: `https://www.fixwheel.app/ghaziabad/${params.locality}`,
    },
    openGraph: {
      type: "website",
      title: `Bike Mechanic Near Me in ${data.name}, Ghaziabad – Doorstep Bike Repair | FixWheel`,
...
export default function GhaziabadLocalityPage({ params }: { params: { locality: string } }) {
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": "Ghaziabad", "item": "https://www.fixwheel.app/ghaziabad" },
      { "@type": "ListItem", "position": 3, "name": data.name, "item": `https://www.fixwheel.app/ghaziabad/${params.locality}` }
    ]
  };
  ...
  const serviceSchema = {
    ...
    "areaServed": { "@type": "Place", "name": `${data.name}, Ghaziabad, Uttar Pradesh` },
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `On average, our mobile mechanics stationed across Ghaziabad reach ${data.name} locations within ${data.eta}.`
        }
      },
      ...data.faqs.map((faq) => ({
        "@type": "Question",
  ...
  return (
    <>
      ...
      />
      <GhaziabadLocalityClientPage slug={params.locality} />
    </>
  );
}
