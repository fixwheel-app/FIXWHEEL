...
export async function generateMetadata({ params }: { params: { locality: string } }): Promise<Metadata> {
  ...
  return {
    title: `Bike Mechanic Near Me in ${data.name}, Delhi – Doorstep Bike Repair | FixWheel`,
    description: `Looking for a bike mechanic near me in ${data.name}, Delhi? FixWheel dispatches certified mechanics to your doorstep in ${data.eta} for oil change, brake repair, battery replacement & full service. Call +91-87459-45682. Starting ₹399.`,
    keywords: [
      `bike mechanic near me in ${data.name}`,
      `bike mechanic near me in ${data.name} Delhi`,
      ...
      `two wheeler mechanic near me in ${data.name} Delhi`,
      ...
      `two wheeler mechanic in ${data.name} Delhi`,
      ...
      `scooter mechanic ${data.name} Delhi`,
      ...
      `Activa 6G service ${data.name} Delhi`,
      `Activa 125 repair near me ${data.name}`,
      `Splendor Plus service ${data.name} Delhi`,
      ...
      `Yamaha R15 V4 service ${data.name} Delhi`,
...
export default function LocalityPage({ params }: { params: { locality: string } }) {
  ...
  const serviceSchema = {
    ...
    "@type": "LocalBusiness",
    "name": "FixWheel",
    "description": `Doorstep bike mechanic in ${data.name}, Delhi. Book engine oil change, brake repair, battery replacement, tyre puncture, and full two-wheeler servicing at home. Covers ${data.subRegionText} with transparent pricing and 15-day warranty.`,
    "url": `https://www.fixwheel.app/delhi/${params.locality}`,
