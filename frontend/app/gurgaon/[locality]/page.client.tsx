...
export default function GurgaonLocalityClientPage({ slug }: LocalityClientProps) {
  ...
  useEffect(() => {
    ...
    getPageVariables('gurgaon/' + slug, 'gurgaon').then(setPageVars);
  }, []);
  ...
  return (
    <div className={`${slug}-scope ${oswald.variable} ${jetbrains.variable}`}>
      ...
      <div className="crumb" style={{ background: "#111214", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "12px 0" }}>
        <div className="wrap">
          <Breadcrumb items={[{ label: "Home", href: "/" }, { label: "Gurgaon", href: "/gurgaon" }, { label: data.name }]} />
        </div>
      </div>
      ...
      <section className="section-alt">
        <div className="wrap">
          <div className="section-head">
            ...
            <h2>Bike Repair Services We Offer in {data.name}, Gurgaon</h2>
          ...
          <div className="why-grid">
            <div className="why-card">
              ...
              <p>Drain old oil, flush engine, and refill with OEM-grade synthetic or mineral oil. Most searched service for bike owners in {data.name} and across Gurgaon. Starting ₹999.</p>
            </div>
            <div className="why-card">
              <span className="num">🛑</span>
            ...
            <div className="why-card">
              ...
              <p>Instant tubeless and tube tyre puncture fix at your home, office, or roadside in {data.name}. No need to push your bike anywhere. Available 7 days a week.</p>
            </div>
          </div>
          <p style={{ marginTop: "24px", fontSize: "14px", color: "var(--ink-dim)" }}>
            Also available: Comprehensive bike service, spark plug replacement, chain & sprocket replacement, air filter cleaning, scooty repair, Royal Enfield servicing, Activa service, and full bike washing in {data.name}, Gurgaon.
          </p>
        </div>
      </section>
