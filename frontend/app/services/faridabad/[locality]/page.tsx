...
const LOCALITIES: Record<string, string> = {
  ...
  "tigaon-road": "Tigaon Road",
  "suraj-kund": "Suraj Kund",
  "mewla-maharajpur": "Mewla Maharajpur",
  "bk-chowk": "BK Chowk",
  "bata-chowk": "Bata Chowk",
  "ymca-chowk": "YMCA Chowk"
};
...
export async function generateMetadata({ params }: { params: { locality: string } }): Promise<Metadata> {
  ...
  return {
    title: `Bike Repair at Home in ${localityName}, Faridabad | FixWheel`,
    description: `Doorstep bike repair in ${localityName}, Faridabad. Verified mechanics at your home or office in ${localityName} & surrounding sectors. Starting ₹499.`,
    alternates: {
      canonical: `https://www.fixwheel.app/services/faridabad/${params.locality}`,
    },
  };
}
...
export default function FaridabadLocalityPage({ params }: { params: { locality: string } }) {
  ...
  // Interlinking: List other localities in Faridabad
  ...
  const faqs = [
    {
      q: `Do you offer bike repair at home in ${localityName} Faridabad?`,
    ...
    },
    {
      ...
      a: `Our mobile mechanics are stationed locally and can reach most parts of ${localityName} within 45-60 minutes of booking confirmation.`
    },
    {
  ...
  return (
    <div className="bg-[#151b24] text-white min-h-screen font-sans">
        ...
        <div className="container mx-auto px-4 relative z-10 max-w-5xl">
          <div className="text-center md:text-left space-y-6">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-accent/10 border border-accent/20 text-accent font-bold uppercase tracking-widest text-[10px]">
              <span>Faridabad Locality Service</span>
            </div>
            ...
            <div className="text-text-secondary text-sm md:text-base leading-relaxed space-y-4 max-w-3xl">
              <p>
                Getting high-quality two-wheeler service in Faridabad is now simpler than ever. FixWheel is proud to offer top-tier doorstep bike repair and maintenance services throughout {localityName}, Faridabad. There is no need to push your broken-down motorcycle or spend hours searching for a trusted mechanic; our background-checked experts come directly to you, providing professional service right at your home or workplace.
              </p>
              <p>
                We cover all key areas of Faridabad, including NIT Faridabad, Sector 15, Ballabhgarh, and Suraj Kund. Whether it is a regular engine oil change, flat tyre repair, or complex electrical troubleshooting, our mechanics carry genuine parts and standard diagnostic gear to ensure a first-class result in {localityName}.
              </p>
              <p>
      ...
      {/* Other Areas We Cover in Faridabad Section */}
      <section className="py-16 bg-[#151b24] border-b border-white/5">
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="text-center md:text-left mb-12">
            <span className="text-xs font-bold uppercase tracking-widest text-accent">Other Localities</span>
            <h2 className="text-2xl md:text-4xl font-black uppercase text-white tracking-tight mt-1">Other Areas We Cover in Faridabad</h2>
          </div>
          <div className="flex flex-wrap gap-2.5">
            {otherLocalities.map((item, idx) => (
              <Link
                key={idx} 
                href={`/services/faridabad/${item.slug}`}
