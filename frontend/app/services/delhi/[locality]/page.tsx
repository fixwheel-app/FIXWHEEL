...
export async function generateMetadata({ params }: { params: { locality: string } }): Promise<Metadata> {
  ...
  return {
    title: `Doorstep Bike Repair in ${data.name} | Two Wheeler Mechanic at Home – FixWheel`,
    ...
    openGraph: {
      ...
      title: `Doorstep Bike Repair in ${data.name} | Two Wheeler Mechanic at Home – FixWheel`,
      description: `Get doorstep repair in ${data.name} for scooters & motorcycles. Verified mechanics offer bike repair in ${data.name} at home — average arrival ${data.eta}.`,
      url: `https://www.fixwheel.app/services/delhi/${params.locality}`,
    },
  };
}
...
export default function LocalityPage({ params }: { params: { locality: string } }) {
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": "Delhi", "item": "https://www.fixwheel.app/services/delhi" },
      { "@type": "ListItem", "position": 3, "name": data.name, "item": `https://www.fixwheel.app/services/delhi/${params.locality}` }
    ]
  };
